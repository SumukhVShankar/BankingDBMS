...
app.get('/customer_details', doesntexist, (req, res) => {
    ...
    let cust,cust2;
    if(p){
        connection.query('select * from customer where cust_id=?',[parseInt(p)],(err,rows)=>{
            if(err) throw err;
             cust=rows;
            res.render('customer_details',{empobj,cust,x});
        });
    }
        connection.query('select * from customer',(err,rows)=>{
            if(err) throw err;
            cust2=rows;
            res.render('customer_details',{empobj,cust2,x});
        });
...
app.post('/customer_details', (req, res) => {
    ...
    {
        res.redirect(`/customer_details?cst=${req.body.recipient}`);
    }
    });
})
...
app.post('/customer_details2', (req, res) => {
    ...
    {
        res.redirect(`/customer_details?cst2=a`);
    }
    });
})
